...
class Collect_Training_ImagesWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    # Create format selector combo box
    self.imageFileFormatSelector = qt.QComboBox()
    self.imageFileFormatSelector.addItems(["Select image file format"])
    self.imageFileFormatSelector.addItems([".jpg",".png",".bmp"])
    parametersFormLayout.addRow(self.imageFileFormatSelector)

    self.referenceNodeSelector = qt.QComboBox()
    self.referenceNodeSelector.addItems(["Select reference node"])
    self.referenceNodeSelector.addItems(["Webcam_Reference","Image_Reference"])
    parametersFormLayout.addRow(self.referenceNodeSelector)
    ...
    self.imageFileFormatSelector.connect('currentIndexChanged(int)', self.onImageFileFormatSelected)
    self.referenceNodeSelector.connect('currentIndexChanged(int)', self.onReferenceNodeSelected)
   ...
   # self.webcamConnectorNode = self.createWebcamPlusConnector() COMMENTED OUT BY OLIVIA 28/02/2019
    #self.webcamConnectorNode.Start() COMMENTED OUT BY OLIVIA 28/02/2019
    #self.setupWebcamResliceDriver() COMMENTED OUT BY OLIVIA 28/02/2019
  ...
  def onImageFileFormatSelected(self):
    #set image file format based on selection from combo box
    self.imageFileFormat = ""
    if self.imageFileFormatSelector.currentText != "Select image file format":
      self.imageFileFormat = self.imageFileFormatSelector.currentText
    else:
      self.imageFileFormat = ".jpg"

  def onReferenceNodeSelected(self):
    #set image file format based on selection from combo box
    self.referenceNode = ""
    if self.referenceNodeSelector.currentText != "Select reference node":
      self.referenceNode = self.referenceNodeSelector.currentText
    else:
      self.referenceNode = "Webcam_Reference"
  ...
  def onStartStopCollectingImagesButton(self):
    ...
    self.logic.startImageCollection(self.collectingImages, self.currentImageClassName,self.currentImageClassFilePath, self.imageFileFormat,self.referenceNode)
...
class Collect_Training_ImagesLogic(ScriptedLoadableModuleLogic):
  def startImageCollection(self,imageCollectionStarted,imageClassName, imageClassFilePath, imageFileFormat, referenceNode):
    try:
    ...
    if self.collectingImages == False:
      self.webcamImageVolume = slicer.util.getNode(self.referenceNode)
  ...
  def onStartCollectingImages(self,caller,eventID):
    ...
    imData = self.getVtkImageDataAsOpenCVMat(self.referenceNode)
    if imData.ndim == 3:
      print("Webcam_Reference")
    elif imData.ndim== 1:
      print("Image_Reference")
    imDataBGR = cv2.cvtColor(imData,cv2.COLOR_RGB2BGR)

    fileName = self.imageClassName + "_" + "%06d" %self.numImagesInFile + self.imageFileFormat
    #if self.numImagesInFile < 10:
      #fileName = self.imageClassName + "_" + "%06d" %self.numImagesInFile + self.imageFileFormat
    #else:
      #fileName = self.imageClassName + "_" + str(self.numImagesInFile) + self.imageFileFormat
